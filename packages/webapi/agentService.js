...
export class AgentService {
  constructor() {
    ...
    this.agentId = process.env.AZURE_SAMUEL_AGENT_ID
  }
  ...
  async processMessage(sessionId, message) {
    try {
      ...
      const createdMessage = await this.client.createMessage(threadId, {
        role: "user",
        content: message,
      });

      let run = await this.client.createRun(threadId, this.agentId);
      
      while (run.status === "queued" || run.status === "in_progress") {
        await new Promise((resolve) => setTimeout(resolve, 1000));
        run = await this.client.getRun(threadId, run.id);
      }
      ...
      const messages = await this.client.listMessages(threadId);
